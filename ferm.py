...
from measures import eq_treatment_measure
...
# from load_compas import CompasLoader
...
if __name__ == "__main__":
    ...
    # dataset_train, dataset_test = CompasLoader()._process_data().load()
    # dataset_train, dataset_test = load_adult(smaller=True)
    parser = argparse.ArgumentParser()
    parser.add_argument('--dataset', type=str, choices=['adult', 'drug', 'compas',
      'arrhythm'], help='Dataset for optimization')
    args = parser.parse_args()
      # dataset_train, dataset_test = load_adult(smaller=True)
      # sensible_feature = 9 # GENDER
      # sensible_feature_values = sorted(list(set(dataset_train.data[:, sensible_feature])))
    dataset_train, dataset_test, sens_feature = load_dataset(args.dataset)
    sensible_feature = sens_feature
    sensible_feature_values = sorted(list(set(dataset_train.sens)))
    # sensible_feature_values = sorted(list(set(dataset_train.data[:, sensible_feature])))
    ...
    EO_train = equalized_odds_measure_TP(dataset_train, clf, [sensible_feature], ylabel=1,
        sens_feats=dataset_train.sens)
    EO_test = equalized_odds_measure_TP(dataset_test, clf, [sensible_feature], ylabel=1,
        sens_feats=dataset_test.sens)
    # ET_test = eq_treatment_measure(dataset_test, clf, [sensible_feature],
        # sens_feats=dataset_test.sens)

    ET_test = eq_treatment_measure(dataset_test, clf, [sensible_feature],
        sens_feats=dataset_test.sens)

    P, FPR, FNR, tpr_sens, fpr_sens, fnr_sens = FP_TP_measure(dataset_test.target,
        pred, sens=dataset_test.sens)
    ...
    print('DFPR test: ', np.abs(fpr_sens[0] - fpr_sens[1]))
    # print('DEOT test: ', np.abs(ET_test[sensible_feature][sensible_feature_values[0]]-
      # ET_test[sensible_feature][sensible_feature_values[1]]))

    #  FERM algorithm
    print('\n\n\nGrid search for our method...')
    sens = dataset_train.data[:, sensible_feature] if isinstance(sensible_feature, int) else dataset_train.sens
    algorithm = FERM(sensible_feature=sens)
    clf = GridSearchCV(algorithm, param_grid, n_jobs=1)
    start = time.time()
    clf.fit(dataset_train.data, dataset_train.target)
    print(f'Elapsed time for FERM fit: {time.time() - start} s')
    ...
    print('Accuracy train:', accuracy_score(dataset_train.target, pred_train))
    # Fairness measure
    EO_train = equalized_odds_measure_TP(dataset_train, clf, [sensible_feature], ylabel=1, sens_feats=sens)
    EO_test = equalized_odds_measure_TP(dataset_test, clf, [sensible_feature], ylabel=1, sens_feats=sens)

    P, FPR, FNR, tpr_sens, fpr_sens, fnr_sens = FP_TP_measure(dataset_test.target,
        pred, sens=dataset_test.sens)
    print('FPR sens: ', fpr_sens)
    print('TPR sens: ', tpr_sens)
    # ET_test = eq_treatment_measure(dataset_test, clf, [sensible_feature], sens_feats=dataset_test.sens)
